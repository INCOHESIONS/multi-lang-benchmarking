...
Here are the recommended versions of all the tools needed to run all the currently available programs:

- .NET 10+
- Bun 1.3.3+
- Clang 21.1.6+ (or any version that supports C++23)
- Go 1.25+
- Java 24+
- Python 3.14+
- Rust 1.84+
...
`benchmarking.py` then loops through each directory, executing `prepare.bat`, which may compile the program in the directory, preferably with optimizations. Then, it executes `run.bat` with 4 arguments: width, height, number of points, and a boolean indicating whether or not to save the image into an /img/ folder. Defaults are `1000`, `1000`, `100`, and `false`. It executes each program 11 times by default, discarding the first round, as it is simply for warm-up.

Then, after tallying all the results, the program generates a `RESULTS.md` containing information about the system, how long each program and run took, and other stats.
