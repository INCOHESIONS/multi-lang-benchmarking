...
import compushady
from compushady.formats import R8_UNORM, R32G32_UINT
...
SHADER = f"""
    ...
    {{
        ...
        for (uint i = 0; i < {number_of_points}u; i++)
            minDist = min(minDist, dot(points[i] - tid.xy, points[i] - tid.xy));

        target[tid.xy] = 1.0F - clamp(sqrt(minDist), 0.0F, 255.0F) * INV_255;
    }}
"""
...
target = compushady.Texture2D(texture_width, texture_height, format=R8_UNORM)
